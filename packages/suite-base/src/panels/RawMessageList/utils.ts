...
/**
 * Checks if the object is an array with exactly one non-undefined element
 */
...
/**
 * If data is a single element array with an object, returns just that object.
 * Otherwise returns the original data.
 */
export const dataWithoutWrappingArray = (data: unknown) => {
  return isSingleElemArray(data) && typeof data[0] === "object" ? data[0] : data;
};

/**
 * Gets the appropriate display value from data
 * Handles single array elements and constant names
 */
export const getSingleValue = (data: unknown, queriedData: any[]): unknown => {
